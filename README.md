...
В этом документе описан протокол взаимодействия с умными замками TTLock. Протокол восстановлен на основе анализа приложения [TTLock](https://play.google.com/store/apps/details?id=com.tongtongsuo.app) для Android.
  ...
  * [Работа с замком](#работа-с-замком)
    + [Подключение к замку](#подключение-к-замку)
...
|4 байта|UID ключа|Нужен для шаринга замков через Интернет, можно один раз сгенерировать случайно|
...
Следующие 4 байта - некий UID ключа. Его возвращает сервер. Мы не работаем с сервером, поэтому, наверное, можно один раз сгенерировать какой-то случайный UID и использовать его во всех последующих командах. В логах, из которых я вытащил команды, UID был 4377454, поэтому в `data` последние 4 байта были `[0x00,0x42,0xcb,0x6e]` (преобразование числа в массив байт здесь происходит так же, как описано в [шаге 3](#3-comm_add_admin))
